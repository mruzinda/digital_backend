...
class DibasParser():
    def __init__(self, dibas_conf_file):
        ...
        for hpc in hpcs:
            ...
            self.dibas_info[hpc]['players'] = []

        self.dibas_info['num_hpcs'] = len(hpcs)

        #roach_config = {}
        players = config.get('DEALER', 'players')
        players = players.split()

        for player in players:
            hpc = config.get(player, 'hpchost')
            hpc_players = self.dibas_info[hpc]['players']
            hpc_players.append(player)
            self.dibas_info[hpc]['players'] = hpc_players
