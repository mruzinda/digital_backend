...
# bit/byte lock helper functions from Luke Hawkins
...
def get_data_sbs(fpga, bb_lock_data_dir, k, j):
    ...
    m = 0
    with open(bb_lock_data_dir + 'lsb_data_re' + str(k) + str(k + 1) + '.txt', 'w') as f:
        for m in range(len(lsb_r)):
            the_str = ""
            the_str += str(lsb_r[m]) + "\n"
            f.write(the_str)
    m = 0
    with open(bb_lock_data_dir + 'lsb_data_im' + str(k) + str(k + 1) + '.txt', 'w') as f:
    ...
    m = 0
    with open(bb_lock_data_dir + 'usb_data_re' + str(k) + str(k + 1) + '.txt', 'w') as f:
        for m in range(len(usb_r)):
            the_str = ""
            the_str += str(usb_r[m]) + "\n"
            f.write(the_str)
    m = 0
    with open(bb_lock_data_dir + 'usb_data_im' + str(k) + str(k + 1) + '.txt', 'w') as f:
