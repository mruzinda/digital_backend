...
class RoachDoctor():
    ...
    def configure(self, simulator=False, bit_lock=False, byte_lock=False, byte_lock_check=False, fft_shift=False, quan_gain=False):
        ...
        for i in range(0, len(self.roaches)):
            ...
            if not simulator:
                ...
                self.write_sim_bram(fpga=roach)
    ...
    def net_config(self, fpga, fid):
        ...
        if self.blackhole:
            for i in range(20):
                fpga.write_int('part2_x_ip' + str(i), _ip_string_to_int(self.blackhole))
    ...
    def write_sim_bram(self, fpga):

        if config_debug:
            print "ConfigFE: Writing simulation BRAMs..."

        baseFilename = './f_engine_config/'

        usb_re_a01 = np.fromfile(baseFilename+'USB_re_sim3.bin', dtype=np.int8)
        usb_im_a01 = np.fromfile(baseFilename+'USB_im_sim3.bin', dtype=np.int8)

        lsb_re_a01 = np.fromfile(baseFilename+'LSB_re_sim3.bin', dtype=np.int8)
        lsb_im_a01 = np.fromfile(baseFilename+'LSB_im_sim3.bin', dtype=np.int8)

        usb_re_a01_packed = struct.pack('>4096b', *usb_re_a01)
        usb_im_a01_packed = struct.pack('>4096b', *usb_im_a01)

        lsb_re_a01_packed = struct.pack('>4096b', *lsb_re_a01)
        lsb_im_a01_packed = struct.pack('>4096b', *lsb_im_a01)

        fpga.write('usb_re_01_1', usb_re_a01_packed, 0)
        fpga.write('lsb_re_01_1', lsb_re_a01_packed, 0)
        fpga.write('usb_im_01_1', usb_im_a01_packed, 0)
        fpga.write('lsb_im_01_1', lsb_im_a01_packed, 0)
