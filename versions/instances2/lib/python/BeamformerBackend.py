...
class BeamformerBackend(VegasBackend):
    ...
    * *unit_test:* Set to true to unit test. Will not attempt to talk to
    ...
    def setFrequencyChunk(self, nchunk):
        """
        Sets the channel chunk index
        nchunk = 0 => channels 0-4
        nchunk = 1 => channels 5-9
        nchunk = 2 => channels 10-14
        nchunk = 3 => channels 15-19
        nchunk = 4 => channels 20-24
    ...
    def start(self, starttime = None):
        ...
        self.write_status(NCHUNK=str(self.nchunk))
    ...
    def configure_roach(self):
        if not self.sim3 and self.has_roaches:
            os.system('bash /users/mruzinda/dibas/lib/python/config_roaches.sh')
    def arm_roach(self):
        # Anish's stuff here
        print "Preparing to arm roach!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
        if not self.sim3 and self.has_roaches:
            os.system('/users/mruzinda/dibas/lib/python/arm.py 1 2 3 4 5 -s 1')
        else:
